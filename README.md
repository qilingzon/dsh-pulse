...
### `verify.mjs`（实测，EXIT=0，305 条断言全 PASS）
...
| 305 + 66 条断言全 PASS | `verify.mjs` / `smoke.mjs` | 断言本身可信；但 smoke 用的是**假 React** |
...
| **其他模型** | ✅ **已测（v0.6.0）**：`r4/deepseek-v4.1-flash` 上阴影仍生效、无假尖峰、误差 −9.4% / −1.8%。但**「按模型分桶」没生效**（该投影在 dock slot 上拿不到），标定是全局共享的 —— 见 `STABILITY.md` §4.1。 |
