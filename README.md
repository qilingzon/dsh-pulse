...
>
> 仓库：<https://github.com/qilingzon/dsh-pulse> ｜ 许可：MIT ｜ 版本：0.4.0
>
...
### `verify.mjs`（实测，EXIT=0，72 条断言全 PASS）
...
- **速度读数在流式段是估算**：`~` 前缀即声明这一点。首步未标定前用种子比 **`0.80 tok/unit`**（四次实机实测的中位数，见《七、数据真实性分级》），实测误差 **−2.0%**；一个步骤结算后即换成本会话实测比并回填本步曲线，偏差不累积。**该种子只对中文为主的技术文体验证过，换模型/语言/文体可能偏离。**
...
环境：`gen4_home` / profile `gen4-lab` / headless Chrome 153 经 CDP 驱动 / 探针读 `data-pulse-*`。
...
**结论一：真实折算比稳定在 0.80 ~ 0.82 tok/unit**（四次：0.801 / 0.824 / 0.799 / 0.816，极差 3%）。
种子从 0.5（拍脑袋）→ 0.62（错把中文标点当非 CJK）→ **0.80（实测）**，误差从 −24% 收敛到 **−2.0%**。

**结论二：宿主流式期确实不报 token 数** —— 四次实测，`decodeTokens` 在流式全程**恒等于基线**，
只在结算瞬间跳到最终值。这不是源码推断，是实机观测。

**结论三：结算假尖峰已消除** —— 未修版在结算瞬间显示 **1115 tok/s 并持续约 20 秒**（10 秒窗把
一次性上报当成瞬时吞吐）；回填版最大速率 **40 tok/s**，`>60 tok/s` 的采样 **0 条**，结算前后为
`36 → idle → 39 → 38`，曲线连续。

复现脚本见 `.diag\dsh-pulse-upload\`（`cdp_measure*.mjs` / `analyze3.mjs` / `truth_ratio.mjs`），未随仓库发布。
...
| 折算比的泛化性 | 三次实测都在同一模型、中文为主、技术文体的输出上。**换模型 / 换语言 / 换文体是否仍接近 0.80，未测。** |
| 多步 turn（含工具调用） | 实测的都是**单步**回答。多步时每一步会重新标定，但「步间切换」的实际观感未观察。 |
| 非 web 客户端 | 只在 web 客户端验过；`client.platform: web` 之外未涉及。 |

> 折算比 0.80 是**四次实测的中位数**，不是普适常数。它只影响**首个步骤**（之后用本会话实测比），
> 且每个步骤结算时都会被回填校正 —— 所以偏差不会累积。
