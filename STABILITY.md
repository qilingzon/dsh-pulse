...
| **v0.7.0** `settings.section` 注册抛错 / slots API 缺失 | **静默降级**：设置页少一个分区，dock 完全不受影响 | 设置里找不到「输出速度曲线」 | 无 `data-pulse-curve` 节点 | 检查 harness 是否改名 |
| **v0.7.0** 轨迹 `localStorage` 被禁 / 配额满 / JSON 损坏 / 版本不符 | 静默降级：曲线只活在内存，重启从零 | 设置页曲线在重启后为空 | `data-pulse-curve="0"` | 无需干预 |
| **v0.7.0** 轨迹为空（刚装上 / 从没对话过） | 画空图 + 「暂无数据」提示，**不画假的 0 线** | 一句说明 + 空坐标轴 | `data-pulse-curve="0"` | 发一条消息即可 |
...
### 4.4 v0.7.0 设置页曲线：一处结构性风险 + 一条诚实备注

**结构性风险：设置页拿不到会话数据。**

`settings.section` 这个槽**不提供会话级投影**（`useProjection` / `useChat` 都拿不到）。
这不是猜的 —— 已装的 `dsh-usage` / `pet` / `session-archive` / `dshmarket` 四个插件，
全是走 `inject` 塞自己的数据源（宿主 RPC 或自持 store），没有一个用投影。

而速度采样活在 dock 组件的 `useRef` 里。**这是本版唯一的结构性风险**：
如果哪一天 `settings.section` 的组件根本挂不上（slot 改名 / 宿主不给注册），
曲线会整个消失 —— 但 dock 那枚 pill 与阴影逻辑**完全不受影响**，
因为两者唯一的耦合点是模块级单例 `sharedTrace`，且注册被 `try/catch` 包住。

**降级路径（已断言）**：
- 拿不到 `slots` API 或 `register` 抛错 → 静默返回空清理函数，设置页少一个分区；
- `localStorage` 被禁 / 配额满 → 曲线只活在内存里，重启后从零开始（不崩、不报错）；
- 轨迹 JSON 损坏 / 版本不符 / 结构被投毒 → 读回空数组（不崩）；
- 无数据 → 画空图 + 「暂无数据」提示，**不画一条假的 0 线**。

**诚实备注：一个没被证实就写进注释的猜测。**

写 `curvePath` 时我注释说「逐字符 `+=` 拼 SVG path 是重画大头」，据此改成数组 + `join`。
`bench.mjs` 实测：**340 µs vs 297 µs，在噪声内** —— 说明大头是 `curveClip` / `curveSpeed`
的数组分配，不是字符串拼接。注释已改回如实描述，代码保留数组写法但不宣称它更快。
这类「先写结论再验证」是本项目反复踩的坑（参见 §2.1 与 README §七 的第三档分级），
所以这一条单独列出来。

**内存增量（唯一显著的一项）**：共享轨迹常驻约 82.5 KiB（1200 点 × 4 数值 + V8 对象头），
跨会话存活；`localStorage` 快照约 58.6 KiB，每 5 s 写一次。对比 dock 的 20 s 缓冲（约 1 KiB），
...
曲线那部分唯一的结构性风险（设置页拿不到会话数据）用模块单例绕开了，代价是约 82.5 KiB 常驻。**
